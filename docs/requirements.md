...
- 環境変数で持つもの:エンドポイント、バケット名、公開URLのベース、リージョン、アクセスキー、シークレットキー
...
- オブジェクトのキーは`{user_id}/{uuid}{拡張子}`。拡張子はcontent typeに応じて`.jpg`/`.png`/`.webp`のいずれか。ユーザーごとに分ける
- 記録を削除したらS3の画像も削除する。`poster_url`が自前のS3のURLかTMDbのURLかを判定する分岐が要る
- 記録の更新でポスターを差し替えたときも、古い画像を削除する。`RecordUsecase.UpdateRecord`で更新前の記録を読み、`poster_url`が変わっていたら古いほうを削除する
